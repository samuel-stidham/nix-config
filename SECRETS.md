...
### Git identity by location

Identity follows the directory tree, one rule per GitHub account, managed through
home-manager `programs.git` in `home/secrets.nix`. `~/Code` is laid out by
identity, not by language or host:

- `~/Code/samuel-stidham/` uses `samuel.stidham@snhu.edu`, the portfolio identity.
- `~/Code/dqfan2012/` uses the global default `dqfan2012@gmail.com`.
- `~/Code/sandbox/` is throwaway, with no identity rule.

The snhu override is a single `includeIf "gitdir:~/Code/samuel-stidham/"`, and its
included file is GENERATED by home-manager through `contents`, so no hand-written
`~/.gitconfig-snhu` exists on disk any more. A repo is therefore correct by
LOCATION and nothing needs a per-repo override. This config lives at
`~/Code/samuel-stidham/nix-config`, so it inherits the snhu email like everything
else under that tree. The old `gitdir:~/nix-config/` rule and the standalone
`~/.gitconfig-snhu` file are both gone, not moved.

dqfan2012 gets no rule of its own. It is still the global default. When that
identity is fully retired, the default flips and the include list stays one line.

### Commit signing

Signing is GPG, not SSH. The base config sets
`user.signingkey = 693484A30BCFADFA` and `commit.gpgsign = true`, so every commit
signs by default, and `home/secrets.nix` keeps this exactly.

One mismatch remains, and it is real. The snhu include overrides only the email,
not the signing key. So commits under `~/Code/samuel-stidham` sign with the
personal GPG key while showing the snhu email. Two ways to resolve it, your choice
in the `ssh-git-identity` phase. Add an snhu signing key to the snhu include, or
...
In this repo, confirm the resolved identity before the first push. The signing key
is GPG, so expect a GPG key id and not an SSH key:

```bash
git -C ~/Code/samuel-stidham/nix-config config user.email      # expect samuel.stidham@snhu.edu
git -C ~/Code/samuel-stidham/nix-config config user.signingkey # expect 693484A30BCFADFA (GPG)
```
